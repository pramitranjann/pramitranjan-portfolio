...
Three read-only, local-only Projects-home concepts live at `/life/calendar-preview`: Atlas maps project relationships, Attention Field plots attention against momentum, and Runway exposes work and deadline collisions over time. The route and its mock components are gitignored, use representative data, and do not mutate the live Projects page.
...
## Open threads
- Pick a Projects-home direction from `/life/calendar-preview`; the lab variants are intentionally prototypes, not a second production route.
...
## Last session
2026-08-12 — Added three read-only Projects-home concepts to `/life/calendar-preview` for visual selection without changing the live route again: a relationship Atlas, an Attention Field, and a time-based Runway. Also rebuilt the live Projects home into a hierarchy-aware index and corrected the Life usability batch: compact quick-add controls, direct Calendar navigation, matched calendar controls, a horizontal desktop event dialog, and List + Horizon Rolodex. TypeScript and whitespace checks pass. Pramit owns the visual QA; no commit or push was performed.
