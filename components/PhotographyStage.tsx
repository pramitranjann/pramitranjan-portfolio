...
export function PhotographyStage() {
  return (
    <section style={{ borderTop: '1px solid #1f1f1f', borderBottom: '1px solid #1f1f1f', display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '200px', alignItems: 'center', padding: '36px 24px', gap: '32px' }}>
      {/* Text left */}
      <div>
        ...
        <h2 className="font-serif italic" style={{ fontSize: '28px', fontWeight: 400, color: '#f5f2ed', lineHeight: 1.2, marginBottom: '16px' }}>
          Before <span style={{ color: '#FF3120' }}>Figma,</span><br />there was <span style={{ color: '#FF3120' }}>film.</span>
        </h2>
        <p className="font-mono" style={{ fontSize: '10px', letterSpacing: '0.08em', color: '#999999', lineHeight: 1.8, maxWidth: '280px' }}>
          Street photography across Southeast Asia. Shot on 35mm and medium format. Always looking.
        </p>
        <Link href="/creative/photography" className="font-mono" style={{ display: 'inline-block', marginTop: '16px', fontSize: '9px', color: '#FF3120', letterSpacing: '0.12em', textDecoration: 'none' }}>
          VIEW ALL →
        </Link>
      </div>
        ...
        <div className="flex" style={{ gap: '3px', padding: '6px 10px', overflow: 'hidden' }}>
          {frames.map((frame) => (
            <div key={frame.label} className="flex-shrink-0">
              ...
              <div className="font-mono" style={{ fontSize: '7px', color: '#FF3120', textAlign: 'center', marginTop: '3px', letterSpacing: '0.1em' }}>{frame.label}</div>
            </div>
          ))}
